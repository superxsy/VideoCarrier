...
def cleanup_task(max_age_hours: int = 24) -> Dict[str, Any]:
    """清理旧文件任务"""
    try:
        logger.info(f"Starting cleanup task, removing files older than {max_age_hours} hours")
        ...
        result = {
            ...
            'size_freed_mb': round(
                (stats_before['total_size_bytes'] - stats_after['total_size_bytes']) / (1024 * 1024), 2
            ),
